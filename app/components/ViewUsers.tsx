    ...
    <div className='flex flex-col items-center scroll-py-5'>
      <div className='flex justify-between  mb-5'>
        ...
        {isLoading ? (
          ...
          <button onClick={handleReload} type='button' className='bg-blue-500 text-white px-2 py-1'>
            Reload
          </button>
        )}
      </div>
      ...
      <div className='w-2/3  px-4 py-2 space-y-4 flex flex-col items-center pb-10 mt-5 border rounded-lg border-gray-700'>
        <div className='w-full space-y-4 flex flex-col items-center pb-10 mt-5'>
          {users.map((users: UserType) => (
            <div key={users.id} className='w-2/3 px-4 py-2 border  rounded-lg border-gray-700'>
        ...
        {/* {users.map((user: UserType) => (
					<Link
						...
						className="flex bornoder-2 rounded-lg border-gray-700 w-full px-2 py-1">
						{JSON.stringify(user)}
					</Link>
				))} */}
      </div>
